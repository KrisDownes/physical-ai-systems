# Hierarchical exploration contract v1 (2026-09-09, pre-model)
...
## Model launch interlock
Requested model remains gpt-5.6-luna / low. One persistent app-server process/thread
and <=20 submitted decision turns are implemented. Installed Codex 0.153.4 exposes
no reliable pre-inference hook covering internal requests/retries/compaction.
**Twenty turns is not twenty actual inference requests. Therefore live LLM mode
fails closed before simulation/model startup.** No API migration or paid calls.
Future pair command is prepared but intentionally blocked until this hard-cap
limitation is resolved; never bypass it and call the result compliant.

Prediction: fewer actual model calls and less measured model-waiting time while
